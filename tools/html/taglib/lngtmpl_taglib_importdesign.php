<?php
   ...
   */
   class lngtmpl_taglib_importdesign extends Document
   {
      ...
      function lngtmpl_taglib_importdesign(){
      }
      ...
      */
      function onParseTime(){
         ...
         if($namespace === null){
            trigger_error('[lngtmpl_taglib_importdesign::onParseTime()] No attribute "namespace" given!',E_USER_ERROR);
            exit();
          // end if
         }
         ...
         if($template === null){
            trigger_error('[lngtmpl_taglib_importdesign::onParseTime()] No attribute "template" given!',E_USER_ERROR);
            exit();
          // end if
         }
         ...
         $this->__parseTranslations($namespace,$template);
      ...
      */
      protected function __parseTranslations($namespace,$template){
         ...
         if($dict_namespace !== null){
            $this->__parseLanguageFile('config::'.$dict_namespace.'::'.$this->__Context,'dictionary');
          // end if
         }

         // use local language file
         $this->__parseLanguageFile($namespace,$template);
      ...
      protected function __parseLanguageFile($namespace,$filename){
         ...
         if(file_exists($fileName)){
            ...
            if($domDoc instanceof SimpleXMLElement){
               $this->__Content = $this->__parseLanguageTokens($this->__Content,$domDoc);
             // end if
            }
            else{
               trigger_error('[lngtmpl_taglib_importdesign::__parseLanguageFile()] The translation file ("'.$filename.'.xml") in namespace "'.$namespace.'" does not contain a valid XML document! The content is returned without translation.');
             // end else
            }
          ...
          // end else
            ...
            trigger_error('[lngtmpl_taglib_importdesign::__parseLanguageFile()] The desired translation file ("'.$filename.'.xml") does not exist in namespace "'.$namespace.'". Please check your tag definition or your configurarion! The content is returned without translation.');
          // end else
         }
      ...
      */
      protected function __parseLanguageTokens($content,$domDoc){
         ...
         for($i = 0; $i < count($translations); $i++){
            $content = str_replace('{{{'.$this->__getAttributeByName($translations[$i],'key').'}}}',$translations[$i][0],$content);
          // end foreach
         }
