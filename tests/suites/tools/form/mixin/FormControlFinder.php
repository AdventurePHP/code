...
trait FormControlFinder {
   ...
   public function &getFormElementByName($name) {
      return $this->getFormElementByAttribute('name', $name);
   }
   ...
   public function &getFormElementByID($id) {
      return $this->getFormElementByAttribute('id', $id);
   }
   ...
   /**
    * @param string $attributeName The name of the attribute to use as selector.
    * @param string $attributeValue The value of the selector attribute.
    *
    * @return AbstractFormControl A reference on the desired form element.
    * @throws FormException In case the form element cannot be found.
    */
   protected function &getFormElementByAttribute($attributeName, $attributeValue) {

      if (count($this->children) > 0) {
         foreach ($this->children as $objectId => $DUMMY) {

            // when we directly find something - get it!
            if ($this->children[$objectId]->getAttribute($attributeName) == $attributeValue) {
            ...
            if ($this->children[$objectId] instanceof FormElementGroup) {
               try {
                  return $this->children[$objectId]->getFormElementByID($attributeValue);
      ...
      throw new FormException('[' . get_class($this) . '::getFormElementByAttribute()] No form element with '
            . $attributeName . ' "' . $attributeValue . '" composed in current form "' . $form->getAttribute('name')
